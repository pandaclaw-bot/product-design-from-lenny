...
- 为什么要加批量筛选模式
- 为什么要把边界收在“定位 + 商业计划书”，而不是一路滑向产品需求文档与实现方案
...
```text
使用 lenny-business-plan 技能，基于已有产品定位，输出一份结构化商业计划书，重点分析市场机会、商业模式、定价、市场进入路径、竞争与风险。
```
...
```text
先阅读 ./skills/lenny-business-plan/SKILL.md，并把它作为本次任务的工作框架。
```
...
```text
先阅读 ./.ai/skills/lenny-business-plan/SKILL.md，再判断这个想法能否成为一门生意，然后再讨论执行计划。
```
...
- 长文初稿：`docs/articles/01-从产品讨论到正式-skill-构建复盘.md`
- 分享短文：`docs/shares/01-两份正式-skill-分享说明.md`
